...
async function scrapeCentreCircleFixtures(email, password) {
  ...
  try {
    ...
    // Try to use installed Chrome, fallback to download if needed
    let launchOptions = {
      headless: true,
      args: ['--no-sandbox', '--disable-setuid-sandbox']
    };
    
    // On Render, Chrome should be installed via `npx puppeteer browsers install chrome`
    try {
      const browserFetcher = puppeteer.createBrowserFetcher();
      const revisions = await browserFetcher.localRevisions();
      console.log('📦 Available Chromium revisions:', revisions);
      
      if (revisions.length > 0) {
        const executablePath = await browserFetcher.revisionInfo(revisions[revisions.length - 1]).executablePath;
        launchOptions.executablePath = executablePath;
        console.log('✅ Using Chrome at:', executablePath);
      }
    } catch (e) {
      console.log('ℹ️ Using default Chrome discovery:', e.message);
    }
