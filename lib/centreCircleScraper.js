...
try {
  ...
  puppeteer = null;
  console.warn('⚠️ Puppeteer not installed. Centre Circle import feature will not be available.');
}
...
async function scrapeCentreCircleFixtures(email, password) {
  if (!puppeteer) {
    throw new Error(
      'Puppeteer is not installed in this environment. ' +
      'The Centre Circle import feature requires browser automation. ' +
  ...
  try {
    ...
    console.log('   - Skip Chromium:', process.env.PUPPETEER_SKIP_CHROMIUM_DOWNLOAD || 'false');
    console.log('   - Executable path:', process.env.PUPPETEER_EXECUTABLE_PATH || 'auto-detected');
    ...
    // Check if we're in an environment that might not have Chrome
    const isProductionRender = process.env.NODE_ENV === 'production' && process.env.RENDER === 'true';
    if (isProductionRender) {
      console.log('⚠️ Running in Render production environment');
      console.log('ℹ️ Attempting to use system Chromium if available...');
    }
    ...
    try {
      ...
      if (errorMsg.includes('Could not find') || errorMsg.includes('Not installed')) {
        const helpMsg = `
⚠️ Chromium browser is not available in this environment.

This feature requires Chromium/Chrome to be installed.

Solutions:
1. For local development: Manually install Chromium via puppeteer (it downloads automatically)
2. For Render deployment: 
   - Install Chromium using a custom build command
   - Or use a different platform with browser support (e.g., Railway, Vercel)
   - Or implement an API-based integration with Centre Circle instead of web scraping

The Centre Circle import feature requires browser automation and cannot run without a browser binary.
        `;
    ...
    throw new Error(`Centre Circle Import Error: ${errorMsg}`);
  }
}
