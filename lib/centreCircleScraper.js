...
async function scrapeCentreCircleFixtures(email, password) {
  ...
  try {
    ...
    if (errorMsg.includes('Could not find Chrome') || errorMsg.includes('Browser was not found')) {
      errorMsg = 'Chromium browser is not available. The server is attempting to download it. Please try again in a moment.';
    }
    
    throw new Error(`Failed to scrape Centre Circle: ${errorMsg}`);
  }
}
